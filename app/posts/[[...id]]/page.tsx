...
async function getRoutePostData(params: Props['params']) {
...
export function generateStaticParams() {
  return getAllPostIds().map(({ params }) => ({
    id: [params.id],
  }));
}
  ...
  return (
    <main className="w-content max-w-full mv-0 m-auto flex flex-col justify-center pt-4 gap-8">
      <div className="grid gap-4">
        <article
          className="card grid grid-cols-[auto_1fr] p-4 gap-2 rounded-lg max-w-full w-full bg-[#212529] border border-neutral-700"
          key={postId}
        >
